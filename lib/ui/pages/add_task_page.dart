...
class _AddTaskPageState extends State<AddTaskPage> {
  ...
  Widget build(BuildContext context) {
    ...
    //_startTime = DateFormat('hh:mm a').format(DateTime.now()).toString();
    ///UPDATE 1: ADDED SWITCH: IS HABIT BOOLEAN
  ...
  _validateInputs() {
    if (_titleController.text.isNotEmpty && _noteController.text.isNotEmpty) {
      _addTaskToDB();
      print(" tasks are added to the database +++++");
      Get.back();
  ...
  //THIS FUNCTION ADDS THE DATA TO THE TASKS DATABASE

  DateTime _currentDate = DateTime.now();

//THIS FUNCTION IS FOR CHECKING THE COUNT OF COMPLETED TASKS SO IF A USERS COMPLETES 5 TASKS A DAY , A DIALOG SHOWS UP FOR HIM

  Future<int?> get_count() async {
    int? count =
        await DBHelper.getTaskCount(DateFormat.yMd().format(_currentDate.toString() as DateTime));
    print(count);
    return count;
  }

  _addTaskToDB() async {
    await _homeController.addTask(
      task: Task(
        ishabit: isHabit == true ? 1 : 0,
    ...
    print(get_count());
    // get_count();
//    int? count = await DBHelper.getTaskCount(DateFormat().format(_currentDate);
